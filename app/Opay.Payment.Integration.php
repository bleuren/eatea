...
class OpaySend extends OpayAio
{
    ...
    protected static function process($arParameters = array(), $arExtend = array())
    {
        ...
        //檢查參數
        $arParameters = self::$PaymentObj->checkString($arParameters);

        //檢查商品
        $arParameters = self::$PaymentObj->checkGoods($arParameters);

        //檢查各付款方式的額外參數&電子發票參數
        $arExtend = self::$PaymentObj->checkExtendString($arExtend, $arParameters['InvoiceMark']);

        //過濾
        $arExtend = self::$PaymentObj->filterString($arExtend, $arParameters['InvoiceMark']);
...
abstract class OpayVerification
{
    ...
    //檢查共同參數
    public function checkString($arParameters = array())
    {
        $arErrors = array();
    ...
    //檢查商品
    public function checkGoods($arParameters = array())
    {
        // 檢查產品名稱。
        $arErrors   = array();
        $szItemName = '';
    ...
    //過濾多餘參數
    public function filterString($arExtend = array(), $InvoiceMark = '')
    {
    ...
    //檢查預設參數
    public function checkExtendString($arExtend = array(), $InvoiceMark = '')
    {
        //沒設定參數的話，就給預設參數
        ...
        //若有開發票，檢查一下發票參數
        if ($InvoiceMark == 'Y') {
            $arExtend = $this->checkInvoicestring($arExtend);
        }
    ...
    //檢查電子發票參數
    public function checkInvoicestring($arExtend = array())
    {
        $arErrors = array();
...
class OpayCVS extends OpayVerification
{
    ...
    public function filterString($arExtend = array(), $InvoiceMark = '')
    {
        $arExtend = parent::filterString($arExtend, $InvoiceMark);
        return $arExtend;
    }
}
